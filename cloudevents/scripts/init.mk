...
# Determine repository root - this allows init.mk to be included from any subdirectory
# First get git root, then append cloudevents subdirectory
GIT_ROOT := $(shell git rev-parse --show-toplevel 2>/dev/null || pwd)
REPO_ROOT := $(GIT_ROOT)/cloudevents
SCRIPTS_DIR := $(REPO_ROOT)/scripts

include $(SCRIPTS_DIR)/docker/docker.mk
include $(SCRIPTS_DIR)/tests/test.mk
-include $(SCRIPTS_DIR)/terraform/terraform.mk
...
githooks-config: # Trigger Git hooks on commit that are defined in this repository @Configuration
	...
	pre-commit install \
		--config $(SCRIPTS_DIR)/config/pre-commit.yaml \
		--install-hooks
