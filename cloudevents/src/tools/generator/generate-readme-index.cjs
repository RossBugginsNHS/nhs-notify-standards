  ...
  for (const entry of entries) {
    if (entry.endsWith("-event.json")) {
      ...
      results.push({
        name: eventName,
        filename: baseName,
        json: `docs/${path.relative(
          DOCS_DIR,
          path.join(exampleEventsDir, entry)
        )}`,
        markdown: `docs/${path.relative(
          DOCS_DIR,
...
function getGeneratedVariants(domain, version, eventBaseName) {
  ...
  if (fs.existsSync(path.join(ROOT_DIR, bundledPath))) {
    variants.push({
      ...
      docs: `docs/${domain}/${version}/events/${eventBaseName}.bundle.schema.md`,
    });
  }
  ...
  if (fs.existsSync(path.join(ROOT_DIR, flattenedPath))) {
    variants.push({
      ...
      docs: `docs/${domain}/${version}/events/${eventBaseName}.flattened.schema.md`,
    });
  }
